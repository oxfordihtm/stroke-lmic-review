#'
#' Screen the search articles using Anthropic's Claude
#' 
#' 

claude_screen_articles <- function(claude_reviewer, query) {
  #screen <- ellmer::chat_claude(system_prompt = context, model = model)
  ...
  out <- claude_reviewer$chat_structured(query, type = type_classification)
...
claude_screen_articles_parallel <- function(claude_reviewer, query) {
  type_classification <- ellmer::type_object(
    "A classification of journal articles on stroke",
    population = ellmer::type_boolean(
      description = "Is the study about a population 18 years and older?"
  ...
  out <- ellmer::parallel_chat_structured(
    chat = claude_reviewer, prompts = query, type = type_classification
  )
